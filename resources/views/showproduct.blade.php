...
@if(Auth::check() && Auth::user()->name)
    ...
    <div class="slider-container">
        ...
        <!-- @if($product->product_image_qr)
            <img src="{{ asset('storage/' . $product->product_image_qr) }}" alt="QR Code">
        @else
...
<script>
    ...
    if (urlParams.get('source') === 'qr') {
        console.log("Halaman ini diakses melalui scan QR Code.");
         // Kirim permintaan POST ke route increment QR Code
        fetch("{{ route('increment.qr.code', $product->id) }}", {
            method: "POST",
            headers: {
            ...
            body: JSON.stringify({
                product_id: "{{ $product->id }}", // Kirim ID produk
            }),
        })
        .then(response => response.json())
        .then(data => console.log(data.message))
        .catch(error => console.error("Error incrementing QR Code scan count:", error));
    }
